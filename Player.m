classdef Player < handle
    ...
    methods
        function Pl = Player(type, realornot, debug)
            ...
            Pl.type = type;
            Pl.real = realornot;
            if Player.perishable;
                Pl.cyanAlwaysSpeculative = Player.forceSpeculativeEquilibriumWhenPerishable;
            end;
            Pl.resetStock();
            Pl.produce();
        ...
        function resetCurrentState(Pl)
            ...
            Pl.alreadyProduced = 0;
            Pl.alreadyPerished = 0;
            Pl.startGood = Pl.myGood();
            Pl.optimalBehavior = '';
            Pl.currentConsumption = 0;
            Pl.currentStockCost = 0;
            Pl.currentProdCost = 0;
            Pl.optimalWill = -1;
            Pl.proposedGood = '';
            Pl.goodExchanged = 0;
            Pl.goodConsumed = 0;
            Pl.newGoodProduced = 0;
            Pl.goodPerished = 0;
            Pl.partnersWillToExchange = '';
            Pl.finalGood = ''; %Pl.newGood='';
            Pl.willToExchange = 0;
            Pl.partner = 0;
            Pl.group = 0;
            Pl.proportionReactionTime = 0;
            Pl.proportionKey = -1;
            Pl.pointsReactionTime = 0;
            Pl.pointsKey = -1;
            Pl.willToExchange = 0;
            Pl.willReactionTime = 0;
            Pl.willConfirmReactionTime = 0;
            Pl.firstWillToExchange = 0;
            Pl.firstWillReactionTime = 0;
            Pl.firstWillConfirmReactionTime = 0;
            Pl.nWillChanged = 0;
        ...
        function Pl = consume(Pl)
            ...
            if Pl.payOwnGood
                command = ['Pl.stock.g', Pl.myTypeGood(), '=Pl.stock.g', Pl.myTypeGood(), '+1;'];
                eval(command); % new instead of Pl.produce();
            else
                Pl.produce();
            end
            Pl.goodConsumed = 1;
        ...
        function Pl = payStock(Pl, R)
            if Pl.real;
                Pl.Ptb.startGoodColor = Pl.myGood();
            end;
            if nargin < 2;
                R = struct;
            end;
            ...
            if (Pl.perishable && ~(strcmp(Pl.perishtype, Pl.goodProduced(Pl.type)) && Pl.alreadyProduced));
                Pl.perishPerishable();
            end;
            Pl.finalGood = Pl.myGood();

            %if Pl.real; fprintf('wallet in payStock= %d. ',Pl.wallet);  disp('\n'); end;
            if Pl.wallet < 0 && Pl.quitIfInsufficientFunds;
                Pl.insufficientFunds = 1;
            end;
            ...
            if Pl.real
                ...
                Pl.Ptb.wallet = Pl.wallet;
                if Pl.simulation == 0;
                    [Pl.pointsKey, Pl.pointsReactionTime] = Pl.Ptb.showPoints(Pl.currentConsumption, Pl.currentStockCost, Pl.currentProdCost, Pl, R);
                end;
            end
        ...
        function Pl = perishPerishable(Pl)
            ...
            if ((strcmp(Pl.perishtype, Pl.goodProduced(Pl.type)) && Pl.alreadyProduced));
                return;
            end;
            perishTypeUGood = Pl.perishtype;
            perishTypeUGood(1) = upper(perishTypeUGood(1));
        ...
        function Pl = exchange(Pl, proposedGood, R)
            ...
            if Pl.real && Pl.simulation == 0;
                Pl.Ptb.animExchange(Pl.partnersType, proposedGood);
            end;
        ...
        function will = shouldIExchange(Pl, proposedGood, R)
            ...
            if strcmp(Pl.type, 'cyan')
                ...
                elseif Pl.perishable && Pl.noStockCost && strcmp(Pl.type, Pl.perishtype)
                    ...
                    r = rand;
                    w(r < percOk) = 1;
                    w(r >= percOk) = 0; %for Magenta (type 3) it is indifferent wether to exchange against yellow or cyan (type 2 or 1) or not;
                    w(Player.forceSpeculativeEquilibriumWhenPerishable > 0) = 0;
                else
                    w = 0;
                end
            end
            if (Pl.noStockCost && myGoodWillPerishAndProposedNot);
                w = 1;
            end;
            if strcmp(myCurrentGood, proposedGood);
                w = 0;
            end;
            if myCurrentGoodIsOfMyType;
                w = 0;
            end;
            if ~(strcmp(Pl.type, 'cyan') && strcmp(proposedGood, 'magenta'));
                Pl.optimalWill = w;
            end;
            if Pl.real && Pl.simulation == 0
                w = Pl.Ptb.shouldIExchange(Pl.partnersType, proposedGood, Proportion, Pl, R);
            end
            if Pl.real && Pl.simulation == 3;
                w = Pl.optimalWill;
            end; %2
            if Pl.real && Pl.simulation == 1;
                w = 1;
            end;
            if Pl.real && Pl.simulation == -1;
                w = 0;
            end;
            if Pl.real && Pl.simulation == 0.5;
                w = 0;
                w((strcmp(Pl.type, proposedGood) || strcmp(Pl.type, proposedGood(2:end))) || Pl.cost(proposedGood) < Pl.cost(Pl.myGood) || myGoodWillPerishAndProposedNot) = 1;
            end;
            will = w;
            if ~Pl.real || Pl.simulation ~= 0;
                endWillTime = GetSecs;
                Pl.overallWillTime = endWillTime - startWillTime;
            end;
        end
