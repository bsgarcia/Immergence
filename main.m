...
if nargin<1; recup=0; end;
KbCheck; WaitSecs(0.001); %preloading functions
...
if ~recup
    ...
    load Round; R.Players(R.numberReal).Ptb.initializeWindow; load hdata; setrandomseed;
end
...
if loadseedformdata; RandStream.setGlobalStream(RandStream('mt19937ar','seed',getseedfromdata(R,'lastGameData',maxRounds))); end;
...
while ~gameEnd
    ...
    if R.nGame==maxNGames; R.lastGame=1; end;
    R.start;
    [headers data]=R.save;
    if R.globalRound==1
        hdata=[headers;data];
    ...
    R.next;
    currentExpTime=GetSecs; elapsedTime=currentExpTime-startExpTime;
    hoursElapsed=elapsedTime/3600;
    if (R.lastGame && R.lastRound); gameEnd=1; end;
    if R.nGame==maxNGames; R.lastGame=1; end;
    if(hoursElapsed>2); R.lastGame=1; R.lastRound=1; end; %the game will stop if after a time-limit in hours (comment to delete the time-limit)
    save hdata hdata;
    %for recuperation:
    save Round R;
end
nowait=0;
if recup>1; nowait=1; end;
compGain=0; hlgain=0; rndStartRound=0; gain10Perc=0; compGain10Perc=0;
if simulation==0
    if realExpAftrSim==0
        [gamePayed gain proba gainEurSup totalGain key time]=R.Players(R.numberReal).Ptb.infoGains(nowait);
            ...
            gain=tenPercInfo(1); compGain=tenPercInfo(2); rndStartRound=tenPercInfo(3); gain10Perc=tenPercInfo(4); compGain10Perc=tenPercInfo(5);
    end
...
datafile=['data_sub' mat2str(R.Players(1).realNumber)]; save(datafile,'hdata');
