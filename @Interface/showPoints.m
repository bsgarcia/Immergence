...
if length(fieldnames(Pl))==0; Pl.insufficientFunds=0; end; %#ok<ISMT>

cgoodColor=this.myGoodColor;
this.myGoodColor=this.startGoodColor;

costword='stockage';
if(prodCost>0); costword='production'; end;
if(prodCost>0 && stockCost>0); costword='stockage et production'; end;
...
if consumption==Pl.utility; mygood=this.type; end;
...
this.myGoodColor=cgoodColor; mygood=this.myGoodColor;
if (strcmp(mygood,Pl.goodProduced(Pl.type))); this.drawSellerPicture(informRect,mygood); end;
...
startLineV=newrect0(RectBottom)+RectHeight(this.rect)*0.005; endLineV=startLineV;
step=RectWidth(newrect0)/40; startLineH=newrect0(RectLeft)+0.05*RectWidth(newrect0); endLineH=startLineH;
while endLineH<newrect0(RectRight)*0.95;
    startLineH=endLineH; endLineH=startLineH+step;
...
if strcmp(mygood(2:end),Pl.perishtype); this.drawSellerPicture(informRect,mygood); vbl=Screen('Flip',this.window,vbl + (0.5)*ifi,1); end;
elapsedSecs=GetSecs()-startSecs;
stillToWait=3.5-elapsedSecs;
if(stillToWait<=0); stillToWait=0.001; end;
if ~this.waitKeyAtGainInfo; WaitSecs(stillToWait); end;
...
%     this.marginText(message_line4,'center',0.75,round(this.fontsize*0.6));
    ...
    if this.waitKeyAtGainInfo; waitNext=0.5; end;
    this.resetScreen();
    normalTextSize=round(this.fontsize*0.9); pointsSize=round(this.fontsize*1.5);
    ...
    if R.lastGame; message_line=sprintf('Un questionnaire va apparaître dans %u secondes.',waitNext); end;
    if this.waitKeyAtGainInfo; 
        message_line='Appuyez sur une flèche gauche ou droite pour continuer';
        if(this.spacesAtInfoScreen); message_line='Appuyez sur la touche espace pour continuer'; end;
