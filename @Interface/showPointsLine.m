...
if nargin<6; imageorsign='sign'; end;
if nargin<5; bottom=0; end;
if nargin<4; rect2=[]; end;
leftMargin=0.05; rightMargin=0.05;
...
if isempty(rect2)
    ...
    rect2=[0 top distanceToRight top+1]; rect2=AdjoinRect(rect2,rect1,RectRight);
end
lineRect=OffsetRect(rect2,0,RectHeight(rect2));
nPoints=txtcell{2};
sign='+'; if nPoints<0; sign='-'; end;
if ~bottom && strcmp(imageorsign,'sign'); nPoints=abs(nPoints); end;
...
leftMarginRect=CenterRect(leftMarginRect,lineRect); rightMarginRect=CenterRect(rightMarginRect,lineRect);
...
if ~isempty(imageorsign) && ~strcmp(imageorsign,'sign'); eval(['signImage=this.Images.' imageorsign ';']); end;
if ~strcmp(imageorsign,'sign'); bottom=1; end;
if bottom; marginrect=ScaleRect(marginrect,lineRectH,lineRectH); end;
offsetTopSign=0.04; if sign=='-'; offsetTopSign=0.2; end; %0
signRect=CenterRect(marginrect,lineRect); signRect=OffsetRect(signRect,0.03*RectWidth(lineRect),offsetTopSign*RectHeight(lineRect)); 
if ~bottom
