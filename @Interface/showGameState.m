function [rep, time] = showGameState(this, Proportion)
    ...
    stateWidth = RectWidth(this.rect);
    stateHeight = RectHeight(this.rect) * 0.20;
    ...
    elemStateW = stateWidth / 5;
    elemStateW = stateHeight * 0.8;
    elemInterval = stateWidth * 0.10;
    elemStateRect = [0, 0, elemStateW, elemStateW];
    elemStateRect = AlignRect(elemStateRect, stateRect, 'bottom', 'left');
    newelemStateRect = elemStateRect;
    unionRect = OffsetRect(newelemStateRect, elemInterval, 0);
    for i = 1:length(this.possibleTypes)
        if i > 1
            newelemStateRect = [0, 0, elemStateW, elemStateW];
            newelemStateRect = AlignRect(newelemStateRect, stateRect, 'bottom', 'left');
        ...
        type = this.possibleTypes{i};
        utype = type;
        utype(1) = upper(utype(1));
    ...
    if ~this.alreadyLaunchedGameState;
        Screen('Flip', this.window);
    end;
