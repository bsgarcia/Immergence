function answer = shouldIExchange(this, othertype, othergood, Proportion, Pl, R)
    ...
    debPlusInfoWaited = 0;
    if (this.debPlusWaitSec ~= 0);
        debPlusInfoWaited = GetSecs() - this.debPlusWaitSec;
    end;
    secsToWait = 1.5 - debPlusInfoWaited;
    if (this.plusWaitTime > 0) 
        secsToWait = this.plusWaitTime;
    end
    if (secsToWait > 0);
        WaitSecs(secsToWait);
    end;
    ...
    Pl.proportionKey = gameStateKey;
    Pl.proportionReactionTime = gameStateTime;
    answer = -1;
    nloop = 0;
    ...
    while answer == -1
        ...
        ctime = 0;
        if nloop == 0
            Pl.firstWillToExchange = rep;
            Pl.firstWillReactionTime = time;
            Pl.firstWillConfirmReactionTime = ctime;
        end
        Pl.willReactionTime = time;
        Pl.willConfirmReactionTime = ctime;
        Pl.nWillChanged = nloop;
        if (isnan(rep));
            rep = 0;
            Pl.nWillChanged = -1;
        end;
