function drawBottom(this,simple,onlyothers)
            ...
            if this.noHintsInBottom; simple=1; end;
            ...
            bottomRect=AlignRect(tempRect,this.rect,'bottom'); if onlyothers; bottomRect=OffsetRect(bottomRect,0,-RectHeight(bottomRect)*0.1); end;
            hintRect=InsetRect(bottomRect,RectWidth(bottomRect)*0.05,RectHeight(bottomRect)*0.07);
            hintElemRect=hintRect; hintElemRect(3)=RectWidth(bottomRect)*0.06+hintElemRect(1);
            hintTextSize=round(this.fontsize*0.6); hintSmallTextSize=round(this.fontsize*0.4);
            if ~onlyothers
            ...
            if length(this.possibleColors)>3; textElemRect=OffsetRect(textElemRect,-RectWidth(textElemRect)*(length(this.possibleColors)-3),0); end;
            for i=1:length(this.possibleColors)
                ctype=this.possibleColors{i};
                textElemRect(2)=hintRect(2); textElemRect(4)=hintRect(4); textElemRect(3)=RectWidth(bottomRect)*0.06+textElemRect(1);
                ...
                wordPoints=' points'; if ccost<2; wordPoints=' point'; end;
