function introduction(this)
    ...
    %etape 2:
    montype = this.findColor(this.type, 1);
    bienproduit = this.findColor(Player.goodProduced(this.type), 1);
    %this.marginText(['Vous êtes un joueur ' montype],0.045,0.045,round(this.fontsize*1.2));
    eval(strcat('bh=this.Images.bh', this.type, ';'));
    rH = RectHeight(this.rect) * 0.4;
    rW = rH * 0.4;
    bhRect = [0, 0, rW, rH];
    bhRect = CenterRect(bhRect, this.rect);
    %Screen('DrawTexture', this.window, bh, [], bhRect);
    %Screen('Flip',this.window,0,0);
    %this.waitKeyPress();
    %etape 3 (Implications):
    rH = RectHeight(this.rect) * 0.58;
    rW = rH * 0.39;
    bhRect = [0, 0, rW, rH];
    ...
    normalTextSize = round(this.fontsize*0.55);
    pointsSize = this.fontsize;
    whiteRect = [0, 0, 0, 0];
    whiteRect(RectLeft) = bhRect(RectRight);
    whiteRect(RectTop) = headerRect(RectBottom) + RectWidth(this.rect) * 0.05;
    whiteRect(RectRight) = this.rect(RectRight);
    whiteRect(RectBottom) = this.rect(RectBottom) - RectWidth(this.rect) * this.headerPercent * 1.3;
