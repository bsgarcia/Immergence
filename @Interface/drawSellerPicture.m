function goodRect = drawSellerPicture(this, rect, good, type, opposite, offsetBall)
    ...
    shrinkSize = RectWidth(rect) * 0.05;
    cadreRect = InsetRect(rect, shrinkSize, shrinkSize);
    goodWidth = RectWidth(cadreRect) * 0.5;
    tempBhRect = ScaleRect(rect, 0.5, 0.90);
    tempGoodRect = [0, 0, goodWidth, goodWidth];
    bhRect = AlignRect(tempBhRect, cadreRect, 'center', 'left');
    goodRect = AlignRect(tempGoodRect, cadreRect, 'right', 'bottom');
    if opposite
        bhRect = AlignRect(tempBhRect, cadreRect, 'center', 'right');
        goodRect = AlignRect(tempGoodRect, cadreRect, 'left', 'bottom');
    end
    if offsetBall ~= 0
