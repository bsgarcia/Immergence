function [gamePayed, tenPercInfo, proba, gainEurSup, hlgain, totalGain, key, time] = infoGains2(this, nowait, R)
    ...
    rndStartRound = 1 + floor(rand()*(R.nRound - tenPercPer));
    rndEndRound = rndStartRound + tenPercPer;
    gain10Perc = 0;
    if use10perc;
        gain10Perc = gameRealWallets(rndEndRound) - gameRealWallets(rndStartRound);
    end;
    display(rndStartRound);
    display(rndEndRound);
    display(gain10Perc);
    display(gameRealWallets(rndStartRound));
    display(gameRealWallets(rndEndRound));
    ...
    if use10perc;
        usedGain = gain10Perc;
        if (gain10Perc < compGain10Perc);
            proba = 100 * gain10Perc / compGain10Perc;
        end;
        if (compGain10Perc < 0 && gain10Perc < 0);
            proba = 100 * compGain10Perc / gain10Perc;
        end;
        if (proba < 0);
            proba = 0;
        end;
        ...
        usedGain = gain;
        if (gain < compGain);
            proba = 100 * gain / compGain;
        end;
        if (compGain < 0 && gain < 0);
            proba = 100 * compGain / gain;
        end;
        if (proba < 0);
            proba = 0;
        end;
    ...
    if exist('HLGain.trg', 'file');
        hlgain = dlmread('HLGain.trg');
        txtHL = ['Vous avez gagné ', mat2str(hlgain), ' euros dans la loterie. '];
...
function [gamePayed, gain] = tirage(this, gameRealWallets, bh, bhRect, nopay, gain10perc, tenPercPer)
    ...
    if nGamesTotal > 1;
        ...
        if ~nopay && this.useTenPercGain;
            gain = gameRealWallets(end);
        end;
    end
