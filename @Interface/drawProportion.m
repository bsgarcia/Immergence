function newrect = drawProportion(this, rect, playersProportions, type)
    ...
    tempBhRect = ScaleRect(rect, 0.4, 0.85);
    bhRect = AlignRect(tempBhRect, rect, 'bottom', 'left');
    bhRect = OffsetRect(bhRect, -RectWidth(bhRect)*0.4, 0);
    Screen('DrawTexture', this.window, bh, [], bhRect);
    leftpos = bhRect(1);
    goodWidth = RectHeight(bhRect) * 1.2;
    goodRect = [0, 0, goodWidth, goodWidth];
    goodRect = AlignRect(goodRect, rect, 'bottom', 'right');
    goodRect = OffsetRect(goodRect, goodWidth*.57, goodWidth*.3);
    shrinkFactor = 0.03; %0.05
    for i = 1:3
        shrinkWidth = RectWidth(bhRect) * shrinkFactor;
        shrinkHeight = RectHeight(bhRect) * shrinkFactor;
        tempBhRect = InsetRect(bhRect, shrinkWidth, shrinkHeight);
        tempBhRect = AlignRect(tempBhRect, bhRect, 'bottom', 'left');
