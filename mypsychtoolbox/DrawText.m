function [rects] = DrawText(wPtr,txt,alignment,fgcol,bgcol,xy)
%DRAWTEXT - One line description goes here.
%   [rects] = DrawText(wPtr, text [,alignment ,fgColor ,bgColor])
%
%   Example
%       >> DrawText
%
...
% Changed 2013-04-08 By Maxim Frolov : returns rectangles, black as the
% default color
% ----------------------------- Script History ---------------------------------
...
for i=1:numel(txt)
    %[xy(i,1) xy(i,2)]=
    [nul rects(i,:)]=Screen('TextBounds',wPtr,txt{i},xy(i,1),xy(i,2)+(i-1)*wh(i,2)/numel(txt));
