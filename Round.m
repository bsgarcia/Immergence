classdef Round <handle
    ...
    methods
        function R=Round(n,debug,simulation,simulationfirst,realExpAftrSim)
            ...
            if(exist('session.exe','file')); R.session=csvread('session.exe'); end;
            R.Players=Players;
            R.nGame=R.nGame+1;
        ...
        function [headers data]=save(R)
            ...
            data=[];
            for i=1:R.nPlayers
                if (R.Players(i).real==0); continue; end; %if activated suppresses the non-real players from the data
                playerType=R.Players(i).type; numPlayer=R.Players(i).number;
            ...
            %wallets:
            grwcount=R.nGame;
            if (R.nGame==1 && R.lastGame); grwcount=R.nRound; end;
            R.gameRealWallets(grwcount)=R.Players(R.numberReal).wallet;
            gameRealWallets=R.gameRealWallets;
            if R.lastRound; save gameRealWallets gameRealWallets; end;
        end
        ...
        function next(R)
            if R.lastRound
                if R.lastGame; return; end;
                R.nGame=R.nGame+1; R.nRound=1; R.lastRound=0;
            else
                R.nRound=R.nRound+1;
            end
        end
        ...
        function start(R)
            ...
            if(prob<R.probabilityEndRound && R.maxRounds==0); R.lastRound=1; end;
            if(R.nRound==R.maxRounds); R.lastRound=1; end;
            ...
            if R.nRound==1 && R.numberReal>0
                maxgroups=5;
                permutationMade=0;
                if(~R.usePredefinedPartners); permutationMade=1; end;
                while ~permutationMade
                    if R.maxRounds~=0; maxgroups=ceil(R.maxRounds/size(R.groupOf6,2)); end;
                    for i=1:maxgroups; if(i==1); R.mancheOrders=R.groupOf6(:,randperm(size(R.groupOf6,2))); else R.mancheOrders=[R.mancheOrders R.groupOf6(:,randperm(size(R.groupOf6,2)))]; end; end;
                    permutationMade=1;
                    if (strcmp(R.mancheOrders{R.nRound}{1},'cyan') && strcmp(R.mancheOrders{R.nRound}{2},'magenta')); permutationMade=0; end;
                    if (strcmp(R.mancheOrders{R.nRound}{1},'yellow') && strcmp(R.mancheOrders{R.nRound}{2},'cyan')); permutationMade=0; end;
                end
            end
            ...
            tgroups=[];
            partnerType='';
            if(R.usePredefinedPartners); partnerType=R.mancheOrders{R.nRound}{1}; end;
            ...
            while goodPartner==0
                ...
                if(strcmp(partnerType,'') || R.numberReal<=0); goodPartner=1; break; end; %if(~R.usePredefinedPartners || R.numberReal<=0)
                s=size(tgroups); ngroups=s(1);
                for g=1:ngroups
                    group=tgroups(g,:);
                    p1=group(1); p2=group(2);
                    if(R.Players(p1).real && strcmp(R.Players(p2).type,partnerType)); goodPartner=1; end;
                    if(R.Players(p2).real && strcmp(R.Players(p1).type,partnerType)); goodPartner=1; end;
                end
            end
            R.groups=tgroups;
            s=size(R.groups); ngroups=s(1);
            if(R.usePredefinedPartners); partnerGood=R.mancheOrders{R.nRound}{2}; end;
            if(R.usePredefinedPartners)
                for g=1:ngroups
                    group=R.groups(g,:);
                    p1=group(1); p2=group(2);
                    if(R.Players(p1).real); R.Players(p1).initPartGood=R.Players(p2).myGood(); R.Players(p2).forceExchange(partnerGood); end;
                    if(R.Players(p2).real); R.Players(p2).initPartGood=R.Players(p1).myGood(); R.Players(p1).forceExchange(partnerGood); end;
                end
            end
            R.computeProportion();
            R.meet();
        ...
        function R=meet(R)
            ...
            for g=1:ngroups
                group=R.groups(g,:);
                p1=group(1); p2=group(2);
                R.Players(p1).group=g; R.Players(p2).group=g;
                R.Players(p1).partner=p2; R.Players(p2).partner=p1;
                R.Players(p1).partnersType=R.Players(p2).type; R.Players(p2).partnersType=R.Players(p1).type;
                good1=R.Players(p1).myGood(); good2=R.Players(p2).myGood();
                R.Players(p1).startGood=good1; R.Players(p2).startGood=good2;
                R.Players(p1).proposedGood=good2; R.Players(p2).proposedGood=good1;
                ...
                R.Players(p1).willToExchange=will1; R.Players(p2).willToExchange=will2;
                R.Players(p1).goodExchanged=0; R.Players(p2).goodExchanged=0;
                R.Players(p1).tellPartnersWill(will2); R.Players(p2).tellPartnersWill(will1);
                %for debug
                if isempty(will1)
                    R.Players(p1)
                    will1
                ...
                %end for debug
               if (will1 && will2)
                   R.Players(p1).exchange(good2,R); R.Players(p2).exchange(good1,R);
               else
                   R.Players(p1).payStock(R); R.Players(p2).payStock(R);
               end
            end
        end
        ...
        function R=computeProportion(R)
            ...
            for t=1:length(possibleTypes)
                ctype=possibleTypes(t); ct=ctype{1,1}; ct(1)=upper(ct(1)); uctype=ct;
                for g=1:length(possibleGoods)
                    cgood=possibleGoods(g); cg=cgood{1,1}; cg(1)=upper(cg(1)); ucgood=cg;
                    for p=1:R.nPlayers
    ...
    methods (Hidden)
        ...
        function R=resetProportion(R)
            ...
            possibleTypes=R.Players(1).possibleTypes;
            for t=1:length(possibleTypes)
                ctype=possibleTypes(t); ct=ctype{1,1}; ct(1)=upper(ct(1));
                for g=1:length(possibleGoods)
                    cgood=possibleGoods(g); cg=cgood{1,1}; cg(1)=upper(cg(1));
